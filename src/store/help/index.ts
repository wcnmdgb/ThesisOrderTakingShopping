...
export function filterRoutesFromLocalRoutes(
  ...
  const filterRoute = localRoutes.find((it) => {
    return resolve(path, it.path) === route.menu_url
  })
  if (filterRoute) {
    filterRoute.meta = {
  ...
  res.forEach((it) => {
    ...
    if (localRoute) {
      ...
      const route: RouteRecordRaw = {
        path: it.outLink && isExternal(it.outLink) ? it.outLink : it.menu_url,
        name: it.routeName || getNameByUrl(it.menu_url),
